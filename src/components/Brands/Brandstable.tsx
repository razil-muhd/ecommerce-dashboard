...
const Brandstables = (brands:any) => {
  const router =useRouter();

  async function deleteBrands(brandId:any) {
   try{
     const deleteBrands = await BrandApi.deleteBrands(brandId);

     console.log("delll:::::::::::",deleteBrands)
     console.log(deleteBrands)
     if(deleteBrands.data.success){
       toast.success(deleteBrands.data.message);
  ...
  return (
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      <div className="max-w-full overflow-x-auto">
       <div className="flex justify-between pb-2"> <h1 className="text-white">Brands</h1>
       <Link className="text-white" href={"/admin/brands/add"}>Add Brands</Link></div>
        <table className="w-full table-auto">
          ...
          <tbody>
          {brands.brands.map((branditem:any, index:number) => (
              <tr key={index}>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === brands.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pr-7.5 ${index === brands.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <button className="hover:text-primary">
                      <Link href={`/admin/brands/edit/${branditem._id}`}>
                    <svg
                    ...
                    <button className="hover:text-primary" onClick={()=>deleteBrands(branditem._id)}>
                      <svg
